...
#################################################################
# LOG 2 DATAFRAME
setwd(file.path("~/cluster/home/aparveen/PKO"))
...
setwd(file.path("/Users/alishaparveen/Documents/PKO_out_no_outlier/count"))
write.xlsx(logDF, "RNA_Log.final.no_out.xlsx", row.names= TRUE)

# LOG 2 GGPLOT
logDF$SAMPLE <- factor(logDF$SAMPLE, levels = sort(logDF$SAMPLE))
ggLog(logDF, "RNA_Log.final.no_out.pdf")
...
# ANNOTATIONS
setwd(file.path("/Users/alishaparveen/Documents/PKO_out_no_outlier/doc")) 
ann <- read.xlsx("RNA_sample_annotation_out.xlsx", sheet = 1) #- #different output name 
...
######################      From Local system           ##################################
##########################################################################################
# Load count matrix directly from local system#
setwd(file.path("/Users/aparveen/Desktop/SEQ/count")) #path
countMat <- read.delim("count.txt")
rownames(countMat) <- countMat[,1]
countMat <- countMat[,-1]

# Load annotation
setwd(file.path("/Users/aparveen/Desktop/SEQ/doc"))
ann <- read.xlsx("RNA_sample_annotation.xlsx", sheet = 1)
ann <- ann[match(colnames(countMat), ann$SAMPLE),]# RE-ORDER annotation

# Remove outliers
#idx <- which(ann$FASTQ == "L10_seq40_S10")
#ann <- ann[-idx,]
...
setwd(file.path("/Users/aparveen/Desktop/SEQ/count")) #path
write.table(rld, "rlogTransformation_DESeq2_out.txt", sep = "\t", quote = FALSE)

deseqDir <- file.path("/Users/aparveen/Desktop/SEQ/count") #path
